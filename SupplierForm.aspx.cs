...
public partial class SupplierForm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["isLogin"] != null)
        {
            ...
            if (isLogin == true)
            {
                var query = from a in context.Categories select a;
                ...
                var aQuery = from a in context.Suppliers select a;
                ...
                foreach (var tmp in aQuery.ToList())
                {
                    ...
                    tableDiv.InnerHtml += "<tr><td class='center' style='text-align: center;'>" + tmp.supplierid + "</td>" +
                                       ...
                                       "<td class='center' style='text-align: center;'>" +
                                          "<a class='btn btn-success customBtn' id='" + tmp.supplierid + "' onclick='View(this.id, \"show\")' href='#editField'>" +
                                            "<i class='glyphicon glyphicon-zoom-in icon-white'></i>" +
                                            "View" +
                                        "</a>" +
                                        "<a class='btn btn-info customBtn' id='" + tmp.supplierid + "' onclick='View(this.id, \"edit\" )' href='#editField'>" +
                                            "<i class='glyphicon glyphicon-edit icon-white'></i>" +
                                            "Edit" +
                                        "</a>" +
                                        "<button class='btn btn-danger customBtn'  id='" + tmp.supplierid + "' onclick='Delete(this.id)' >" +
    ...
    public static string Delete(string tmpid)
    {
        ...
        {
            if (query.supplierid == tmp.supplierid)
            {
                return String.Format("The {0} supplier is still in use", query.companyname);
            }
        }
    ...
    public static string Search(string company)
    {
        ...
        {
            text += "<tr><td class='center' style='text-align: center;'>" + tmp.supplierid + "</td>" +
                               "<td class='center' style='text-align: center;'>" + tmp.companyname + "</td>" +
